...
\noindent\fbox{\parbox{0.96\columnwidth}{\small\textbf{T01 working-draft status.}
The equations, proposition, proof, four-stage method, and planned evaluation are
carried over from the frozen structure.  T00 has verified only the legacy backend
build and one repeatable legacy run.  The NLOS module, paper runner, locked
calibration, and RQ1--RQ4 experiments remain unimplemented or unrun.}}

\begin{abstract}
Offline UWB--inertial post-processing can exploit future measurements to revise historical estimates, but it also needs explicit control of calibration, robustness, uncertainty diagnostics, and evaluation provenance. This T01 working draft defines the planned UWB-IMU-IE system and its evidence contract. The frozen NLOS method uses nonnegative temporally structured support discovery, an unregularized constrained segment refit, candidate-excluded marginal information, and a frozen group-level use-or-suppress rule. Accepted bias states remain in the intended final joint graph rather than becoming exact corrections. At this revision, only the legacy backend build and one repeatable legacy run have been verified; the NLOS implementation and RQ1--RQ4 results are explicitly pending. \todo{Replace this status text only with locked implementation and experiment evidence; do not invent results.}
\end{abstract}
...
\textbf{C2---Focused processing method.} The frozen design is a segment-level NLOS module combining adopted support discovery, debiased refitting, candidate-excluded marginal-information assessment, and frozen use/suppress decisions with uncertainty-preserving final inference. It is not implemented at T01.
...
\label{sec:method}
This section preserves the frozen method definition. Its equations and proposition
specify the implementation target; they are not evidence that Stages 1--4 are
present in the repository. The implementation status and unresolved parameters
are tracked in the T01 method contract.
