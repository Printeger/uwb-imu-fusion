...
\noindent\fbox{\parbox{0.96\columnwidth}{\small\textbf{0911 post-fit FDE v2 engineering status.}
A checked raw Gaussian reference and a paper-only robust-loss consistency fix
restore full Walk1 localization (legacy/all-range/Cauchy aligned RMSE:
0.169642/0.163853/0.163847\,m). Complete Gaussian post-fit normalization and
audited zero-optimizer empty-support reuse pass 29 CTests, the fixed smoke,
and all five full Walk1 methods. Both runs have zero candidates; they do not
establish correction benefits. The historical six-input failures remain valid
historical evidence; other inputs receive prepare checks only in this revision.}}

\begin{abstract}
Offline UWB--inertial post-processing can exploit future measurements to revise historical estimates, but it also needs explicit control of calibration, robustness, uncertainty diagnostics, and evaluation provenance. The implemented paper path uses an IMU-aided residual fault-detection-and-exclusion (FDE) front end, temporal positive-excess aggregation, an unregularized constrained segment refit, candidate-excluded marginal information, and frozen final policies. Accepted live bias states remain in the intended final joint graph; the LCB variants instead apply their separately specified fixed offsets. This residual front end belongs to the RAIM/FDE family, but it is neither a complete ARAIM implementation nor a certified integrity method. The historical six-input evidence is negative/incomplete. The present Walk1 localization recovery and empty-support results establish engineering behavior, with no evidence of candidate correction benefits.
\end{abstract}
...
First solve the complete Gaussian raw-range/IMU reference graph with checked
LM. Its Values initialize robust baselines and serve directly as the all-range
and FDE reference. Linearize that same graph with all actual unknowns and full
IMU covariance whitening. For whitened Jacobian $A$ and scalar range row $j(i)$,
\begin{align}
 d_i&=[I-AA^\dagger]_{j(i),j(i)},\quad [Q_r]_{ii}=\sigma_i^2d_i,\\
 r_i&=h_i(\widehat\X)+\beta_{m(i)}-z_i,\quad
 w_i=\frac{r_i}{\sqrt{[Q_r]_{ii}}},\quad T_i=w_i^2.
 \label{eq:discovery}
\end{align}
This is the local Gaussian residual covariance and local test normalization
\cite{kuusniemi2005}. A single sparse QR and selected unit-vector solves compute
the diagonals without a dense inverse or full projector. Uncertain rank or
unresolved variance fails explicitly; damping and artificial priors are absent.
The measurement-normalized $r_i/\sigma_i$ is a separately named diagnostic.
Data-dependent initialization priors and calibration gaps remain limitations;
a per-observation one-percent level is not a sequence-level false-alarm rate.
...
If FDE succeeds with all planned observations tested and an empty partition,
\texttt{SUCCESS\_EMPTY} reuses the audited reference graph and Values. Stage~2
and the four identical candidate-policy finals invoke no optimizer; alternating
refit stopping conditions are not applicable. A nonempty partition with all
candidates suppressed retains the original final/fallback path.
...
\bibitem{kuusniemi2005}
H. Kuusniemi, ``User-Level Reliability and Quality Monitoring in Satellite-Based Personal Navigation, Ph.D. dissertation, Tampere University of Technology, 2005, Sec.~5.1, Eqs.~54 and~60.
\bibitem{forster2015}
