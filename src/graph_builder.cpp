...
void GraphBuilder::AddUwbFactorsForFrame(size_t kf_idx, const UwbFrame& frame,
                                         NonlinearFactorGraph* graph,
                                         std::vector<size_t>* uwb_indices,
                                         Values* values, bool first_frame) {
...
void GraphBuilder::Build(const std::vector<UwbFrame>& uwb_kf,
  ...
  for (size_t k = 1; k < N; ++k) {
    ...
    gtsam::NavState pred =
        pim.Pim().predict(gtsam::NavState(prev_pose, prev_vel), prev_bias);
  ...
  std::cout << "GraphBuilder: " << N << " keyframes, " << graph->size()
            << " factors (" << uwb_indices->size() << " UWB).\n";
}
