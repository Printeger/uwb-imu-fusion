...
Config ConfigLoader::Load(const std::string& yaml_path) {
  ...
  if (node["extrinsics"]) {
    ...
    if (ex["lever_prior_sigma"]) cfg.lever_prior_sigma = ex["lever_prior_sigma"].as<double>();
  }
  ...
  if (node["calibration"]) {
    ...
    if (cal["calib_range_bias"])  cfg.calib_range_bias  = cal["calib_range_bias"].as<bool>();
    if (cal["range_bias_sigma"])  cfg.range_bias_sigma  = cal["range_bias_sigma"].as<double>();
  ...
  if (node["uwb"]) {
    ...
    if (uw["nlos_rssi_diff"])          cfg.nlos_rssi_diff          = uw["nlos_rssi_diff"].as<double>();
    if (uw["min_range"])               cfg.min_range               = uw["min_range"].as<double>();
    if (uw["max_range"])               cfg.max_range               = uw["max_range"].as<double>();
    if (uw["dist_consistency_thresh"]) cfg.dist_consistency_thresh = uw["dist_consistency_thresh"].as<double>();
  ...
  if (node["solver"]) {
    ...
    if (sv["chi2_reject_prob"])    cfg.chi2_reject_prob    = sv["chi2_reject_prob"].as<double>();
    if (sv["max_rejection_rounds"]) cfg.max_rejection_rounds = sv["max_rejection_rounds"].as<int>();
  }
...
std::string ConfigLoader::ResolveBagPath(const std::string& config_dir,
  ...
  fs::path data_rel = pkg_root / "data" / fs::path(bag_path).filename().string();
