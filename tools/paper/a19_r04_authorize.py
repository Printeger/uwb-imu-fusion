#!/usr/bin/env python3
"""Consume the sole R04 science ticket under separate automatic/final limits."""
...
def main() -> int:
    ...
    if list(evidence.glob("attempts/*/TICKET.json")):
        raise SystemExit("R04_SCIENCE_TICKET_ALREADY_CONSUMED")
    for row in preflight["identities"]:
        path = Path(row["path"])
        if hashlib.sha256(path.read_bytes()).hexdigest() != row["sha256"]:
            raise SystemExit("IDENTITY_CHANGED_AFTER_PREFLIGHT:" + str(path))
    prepared = prepare_attempt(evidence, "attempt1")
    ticket = {
        "schema": "A19_R04_AUTO_SCORE_AND_FINAL_TICKET_V1",
        "attempt": "attempt1", "role": "development",
        "scenario": "A10_FROZEN_P1_STEP_SEED10101",
        "initialization": "ORIGINAL_RAW_NO_CHECKPOINT",
        "stage1_outer_max": 500, "conditional_block_iteration_max": 50,
        "stage2_outer_max": 200, "automatic_hard_timeout_seconds": 900,
        ...
        "truth_gt": "FORBIDDEN", "retry": "FORBIDDEN_AFTER_ALGORITHM_START",
    ...
    command = [
        ...
        str(prepared["root"] / "estimator_run"), "--seconds", "5400", "--",
        str(runner), "--policy",
    ...
    write_json_atomic(prepared["root"] / "COMPLETION.json", {
        "schema": "A19_R04_COMPLETION_V1", "attempt": "attempt1",
        "wrapper_exit": completed.returncode, "actual_exit": actual["exit_code"],
