...
std::vector<DevelopmentRangeConstant> ranges(const A17Graph&fx){std::vector<DevelopmentRangeConstant>v;for(auto&kv:fx.ranges){auto&m=kv.second;v.push_back({kv.first,fx.graph[kv.first]->keys()[0],m.at("anchor"),m.at("lever"),m.at("measurement")(0,0),1.0/boost::dynamic_pointer_cast<noiseModel::Gaussian>(boost::dynamic_pointer_cast<NoiseModelFactor>(fx.graph[kv.first])->noiseModel())->R()(0,0),m.at("beta")(0,0),0});}return v;}
