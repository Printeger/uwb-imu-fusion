...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const bool prepare_only = mode == "development-prepare-only";
    if (!prepare_only && mode != "development-stage1-stage2-score")
    ...
    const std::string sid = Strategy19(config);
    {
      ...
      f << "{\"schema\":\"" << kA19Schema
        ...
        << sid << "\",\"implementation\":\"A19_SHARED_DEVELOPMENT_IDENTITY_CMAKE_ABI_MATCHED_MPFR_333_INEXACT_HANDOFF_FINAL_EMPTY_CERTIFIED_V1\"}\n";
    }
    ...
    auto stage1_request = MakeA19DevelopmentStage1Request(sid, [&](
    ...
    }
    {
      ...
      frozen << "{\"schema\":\"A19_R04_FROZEN_DECISIONS_V1\","
      ...
      for (const auto& spec : kFinalPolicies) {
        ...
        if (child == 0) {
          ...
          try {
            ...
            empty_recovery_request.role = "development";
