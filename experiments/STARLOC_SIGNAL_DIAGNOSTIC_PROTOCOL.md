...
为回答用户第5项，只在此前直接报BOOTSTRAP_IMU_EMPTY的zigzag_s4上增加一次裁剪对照。仅删除校正后首IMU之前23条tag1 UWB，输入保持旧offset/旧轴/旧噪声和所有参数，不应用本次信号诊断拟合。保留原始source ID，重建共同input plan及初始化。新输出独立；先prepare/ROS/probe，再依次producer、Cauchy、SFUISE各一次、每树1800s，无重试，不启动RR finals。目的分别验证bootstrap错误是否解除及基础定位是否改善，不作为新RR矩阵。新旧Cauchy/SF在裁剪后首UWB起10Hz共同有效时刻分别一次SE3对齐，所有失败NA保留。其他两条只报告裁剪边界，不扩大运行。
