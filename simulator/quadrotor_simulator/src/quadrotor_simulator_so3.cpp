  ...
  */
  ...
  while (n.ok()) {
    ...
    if (tnow >= next_odom_pub_time) {
      ...
      imu_pub.publish(imu);
      // Publish TF so RViz can visualize the drone pose
      {
        tf::Transform t;
        t.setOrigin(tf::Vector3(state.x(0), state.x(1), state.x(2)));
        Eigen::Quaterniond q(state.R);
        t.setRotation(tf::Quaternion(q.x(), q.y(), q.z(), q.w()));
        tf_br.sendTransform(tf::StampedTransform(t, tnow, "world", quad_name));
      }
    }
