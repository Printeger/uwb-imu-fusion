...
%
% BUILD: pdflatex UWB_IMU_IE_System_Centered_Structure_v3.tex (twice).
...
\begin{abstract}
Offline UWB--inertial post-processing can exploit future measurements to revise historical estimates, but it also needs explicit control of calibration, robustness, uncertainty diagnostics, and evaluation provenance. This paper presents the structure of UWB-IMU-IE, a full-trajectory inference system organized around those requirements. Its focused NLOS module adopts a nonnegative temporally structured bias model for support discovery, refits segment amplitudes without sparsity shrinkage, and assesses whether candidate biases can be separated from uncertain navigation states. A rank-aware marginal-information diagnostic, an absolute uncertainty check, and an empirical model-fit check govern a frozen use-or-suppress decision. Accepted bias states remain in the final joint graph rather than being treated as exact corrections. Evaluation is organized around system reproducibility, controlled persistent multi-link obstruction, held-out decision validity, and matched future-context experiments. \todo{Replace planning language with verified implementation, quantitative findings, and actual artifact availability; do not invent results.}
\end{abstract}
...
\paragraph{Contributions, in this order.}
\textbf{C1---System capability.} A reproducible full-trajectory raw-range UWB--inertial pipeline with interchangeable processing policies and graph-consistent diagnostic outputs. Only implemented, tested, and released capabilities enter this claim.
\textbf{C2---Focused processing method.} A segment-level NLOS module combining adopted support discovery, debiased refitting, candidate-excluded marginal-information assessment, and frozen use/suppress decisions with uncertainty-preserving final inference.
\textbf{C3---Evidence and artifact.} A controlled evaluation separating system portability, empirical decision quality, multi-link NLOS performance, and future-context effects, with traceable commands and configurations.
...
The main experiments assume surveyed anchors, a calibrated lever arm, synchronized clocks, and an independent LOS calibration of static link bias. Optional estimated calibration states remain system extensions, not headline contributions. No real-time guarantee, fixed-lag implementation, new sensor integration, or full UAV validation is required by this paper.

The essential interface is
\emph{recording + calibration + configuration $\rightarrow$ final graph/state + diagnostics + manifest}.
A manifest records input and configuration hashes, software revision, seeds, units/frames, keyframe policy, solver settings, candidate/support masks, accepted groups, calibration provenance, and output status. Calibration and uncertainty assumptions are part of the experiment, not hidden preprocessing.
...
\plan{Describe data adapters, initialization, factor construction, policy plug-ins, and diagnostics in one paragraph each only if needed.}
At minimum, validate the same backend on simulation, the controlled UGV data, and \emph{one} external UWB--IMU dataset. A second public dataset is optional. Report ingestion success, failure reasons, runtime by stage, peak memory, and rerun agreement within declared tolerances. Do not promise bitwise equality from parallel numerical solvers.

\subsection{Artifact contract}
The intended release contains source, build environment, tests, one example dataset/configuration, baseline commands, masks, manifests, and scripts regenerating the reported results. Distinguish \emph{implemented}, \emph{tested}, and \emph{released}. \todo{Insert the actual anonymous repository/archive only after it exists; restore ``open-source'' to the title only when supported.}

\section{Recoverability-Guided NLOS Processing}
\label{sec:method}
\subsection{Stage 1: adopted support discovery}
Write $J_0=J_{\mathrm{prior}}+J_{\mathrm{IMU}}+J_\beta$ and $w_i=\sigma_i^{-2}$, with $i=(k,m)$. Discover candidate support using
\begin{equation}
 \begin{aligned}
 \min_{\X,\bm\beta,\mathbf b\geq0}\quad
 &J_0+\frac12\sum_{i\in\E}w_i(h_i+\beta_{m(i)}+b_i-z_i)^2\\
 &+\lambda_1\|\mathbf b\|_1+\lambda_{\rm TV}\|D\mathbf b\|_1.
 \end{aligned}
 \label{eq:discovery}
\end{equation}
Here $D$ differences consecutive valid samples \emph{within each link}. Split chains at packet gaps exceeding a fixed threshold; specify time/gap conventions. Freeze activity thresholds, minimum segment sample count/duration, and change-point rules using validation data. These choices determine the model and must not be tuned on test labels.
...
\State Discover support using Eq.~\eqref{eq:discovery}
